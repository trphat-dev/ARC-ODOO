...
export class ReportPurchaseContractWidget extends Component {
    static template = xml`
            <div class="report-contract-statistics-container">
                <!-- Header -->
                <div class="report-contract-statistics-header">
                <h1 class="report-contract-statistics-title">Báo cáo Hợp đồng Mua</h1>
                <p class="report-contract-statistics-subtitle">Thống kê hợp đồng mua</p>
                </div>
                    ...
                    <table class="report-contract-statistics-table">
                        <thead>
                            <tr>
                                ...
                                <th style="font-size: 0.8rem; font-weight: 600; width: 120px;">Ngày HĐ mua</th>
                                ...
                                <th style="font-size: 0.8rem; font-weight: 600; width: 100px;">Số ngày tham gia</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 150px;">Tiền lãi dự kiến</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 150px;">Gốc + lãi dự kiến</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr t-if="!state.records || state.records.length === 0">
                                <td colspan="13" class="text-center py-3" style="font-size: 0.85rem;">
                                    <i class="fas fa-inbox me-2"></i>Không có dữ liệu hợp đồng mua
                                </td>
                            </tr>
                            <tr t-foreach="state.records" t-as="record" t-key="record.id">
                                ...
                                <td style="font-size: 0.8rem;" t-esc="record.ngay_hd_mua || ''"/>
                                ...
                                <td style="font-size: 0.8rem;">
                                    <span class="chip chip-days" t-esc="record.so_ngay_tham_gia || 0"/>
                                </td>
                                <td style="font-size: 0.8rem;" class="text-right price-cell" t-esc="this.formatNumber(record.tien_lai_du_kien)"/>
    ...
    setup() {
        ...
        onMounted(() => {
            console.log('ReportPurchaseContractWidget OWL component mounted');
            this.initDropdown();
    ...
    async loadData() {
        ...
        try {
            const response = await this.rpc('/report-purchase-contract/data', {
    ...
    async exportPdf() {
        ...
        try {
            ...
            const response = await fetch(`/report-purchase-contract/export-pdf?${params.toString()}`, {
                method: 'GET',
                headers: {
            ...
            a.href = url;
            a.download = `report_purchase_contract_${new Date().toISOString().split('T')[0]}.pdf`;
    ...
    async exportXlsx() {
        ...
        try {
            ...
            const response = await fetch(`/report-purchase-contract/export-xlsx?${params.toString()}`, {
                method: 'GET',
                headers: {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `report_purchase_contract_${new Date().toISOString().split('T')[0]}.xlsx`;
            document.body.appendChild(a);
            a.click();
            window.URL.revokeObjectURL(url);
            document.body.removeChild(a);
