...
export class ReportSellContractWidget extends Component {
    static template = xml`
        <div class="report-contract-statistics-container">
            <!-- Header -->
            <div class="report-contract-statistics-header">
                <h1 class="report-contract-statistics-title">Báo cáo Hợp đồng Bán</h1>
                <p class="report-contract-statistics-subtitle">Thống kê hợp đồng bán</p>
            </div>
                    ...
                    <table class="report-contract-statistics-table">
                        <thead>
                            <tr>
                                ...
                                <th style="font-size: 0.8rem; font-weight: 600; width: 120px;">Ngày HĐ bán</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 120px;">Ngày bán</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 120px;">Ngày thanh toán</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 100px;">Số ngày duy trì</th>
                        ...
                        <tbody>
                            <tr t-if="!state.records || state.records.length === 0">
                                <td colspan="11" class="text-center py-3" style="font-size: 0.85rem;">
                                    <i class="fas fa-inbox me-2"></i>Không có dữ liệu hợp đồng bán
                                </td>
                            </tr>
                            <tr t-foreach="state.records" t-as="record" t-key="record.id">
                                ...
                                <td style="font-size: 0.8rem;" t-esc="record.ngay_hd_ban || ''"/>
                                <td style="font-size: 0.8rem;" t-esc="record.ngay_ban || ''"/>
                                <td style="font-size: 0.8rem;" t-esc="record.ngay_thanh_toan || ''"/>
                                <td style="font-size: 0.8rem;" t-esc="record.so_ngay_duy_tri || ''"/>
                                <td style="font-size: 0.8rem;" class="text-right price-cell" t-esc="this.formatNumber(record.tien_lai)"/>
                                <td style="font-size: 0.8rem;" class="text-right price-cell" t-esc="this.formatNumber(record.goc_lai)"/>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    setup() {
        ...
        onMounted(() => {
            console.log('ReportSellContractWidget OWL component mounted');
            this.initDropdown();
    ...
    async loadData() {
        ...
        try {
            const response = await this.rpc('/report-sell-contract/data', {
    ...
    async exportPdf() {
        ...
        try {
            ...
            const response = await fetch(`/report-sell-contract/export-pdf?${params.toString()}`, {
                method: 'GET',
                headers: {
            ...
            a.href = url;
            a.download = `report_sell_contract_${new Date().toISOString().split('T')[0]}.pdf`;
    ...
    async exportXlsx() {
        ...
        try {
            ...
            const response = await fetch(`/report-sell-contract/export-xlsx?${params.toString()}`, {
                method: 'GET',
                headers: {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `report_sell_contract_${new Date().toISOString().split('T')[0]}.xlsx`;
            document.body.appendChild(a);
            a.click();
            window.URL.revokeObjectURL(url);
            document.body.removeChild(a);
